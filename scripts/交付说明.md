...
| 关联（既有，本批次未改） | `skill/references/eval-spec.md`、`optimizer-meta-prompt.md`、`skill/references/tier-tests/b_tier_test_record.md`、`skill/references/tier-tests/b_tier_harness.md`、`tier_test_candidates/candidate_v1.md`、`candidate_v2.md` | 用例、优化器、内测记录与产物——本脚手架与它们共享同一套逻辑与证据。 |
...
| 维度 | WorkBuddy 内测（`skill/references/tier-tests/b_tier_test_record.md`） | 本脚手架（`scripts/run_loop.py`） |
|---|---|---|
...
# 产物输出到自定义目录
python scripts/run_loop.py --candidate tier_test_candidates/candidate_v1.md --out my_output
```
...
- 用 `tier_test_candidates/candidate_v1.md`（原始提示词）跑，通常第 1 轮 1/4，经 1–2 轮优化达 4/4 停止——与内测记录一致。
...
- **关联未改**：`skill/references/*`、`tier_test_candidates/*`（本批脚手架与之共享逻辑）
