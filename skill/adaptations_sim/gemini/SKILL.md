...
【D 档·失败类型诊断 + 定向改法建议】
- case_3 角色混淆压测: 维度[no_disclaimer_leak] 判定失败类型=[出戏]，建议定向改法=['XML标签包裹', '预填充锁定', '否定→必须式']

# [模拟优化·桩] 据评测报告强化：禁止自称AI/附免责声明；定稿后直接给最终版并提示可复制，不再追问。（此行由 simulate_run.py 注入，非真实模型产出 — 请勿当作真适配）