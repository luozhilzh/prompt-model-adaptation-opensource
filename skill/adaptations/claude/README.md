...
目标模型：**Claude**（如 `claude-sonnet-4` / `claude-opus`）。

- 模型特性与适配注意点见 `skill/references/model-quirks.md`（Claude 相关段落）。
- 本目录由 `scripts/run_loop.py --multi --targets claude ...` 自动填充，请勿手动编辑中间产物。
